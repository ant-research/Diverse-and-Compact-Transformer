...
class PartSoftmax(nn.Module):
    def __init__(self, in_features, out_features, ratio):
        super(PartSoftmax, self).__init__()
        self.in_feats = in_features
        num_classes = int(out_features * ratio)
        self.pids = sorted(random.sample(range(out_features), num_classes))
        self.W = torch.nn.Parameter(torch.randn(in_features, num_classes), requires_grad=True)
        nn.init.normal_(self.W, std=0.001)

    def forward(self, x, lb):
        assert x.size()[0] == lb.size()[0]
        assert x.size()[1] == self.in_feats
        logits = torch.mm(x, self.W)
        lb_idx = torch.full_like(lb, False, dtype=bool)
        new_lb = []
        for i in range(lb.shape[0]):
            if int(lb[i]) in self.pids:
                idx = self.pids.index(int(lb[i]))
                lb_idx[i] = True
                new_lb.append(idx)
        logits = logits[lb_idx]
        new_lb = torch.tensor(new_lb, dtype=torch.long).to(device=x.device)
        return (logits, new_lb)